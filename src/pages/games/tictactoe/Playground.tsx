...
import { initializeStats, updateStatsWithMove } from "@/utils/statsManager";
...
const Index = () => {
  ...
  const clearAllData = () => {
    ...
    // Reset stats
    setModel1Stats(initializeModelStats(""));
    setModel2Stats(initializeModelStats(""));
    ...
    <div className="min-h-screen bg-cyber-dark bg-cyber-gradient relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-4 relative z-10">
        <header className="mb-4">
          <h1 className="text-2xl md:text-4xl font-bold text-cyber-light mb-2 animate-cyber-pulse">
            The Super One
          </h1>
          <p className="text-cyber-accent text-sm md:text-base">
            AI Model Arena
          </p>
        </header>

        <div className="flex flex-col lg:flex-row gap-4">
          {/* Left Side - Model X Config or Stats */}
          <div className="w-full lg:w-1/4">
            {!gameInProgress && !gameOver ? (
              <div className="bg-cyber-primary/80 backdrop-blur-sm rounded-lg p-4 border border-cyber-accent/30 shadow-cyber hover:shadow-cyber-glow transition-all duration-300">
                <h2 className="text-xl font-bold text-cyber-light mb-3 flex items-center">
                  <span className="text-cyber-accent mr-2">Model X</span>
                  <div className="w-2 h-2 bg-cyber-accent rounded-full animate-cyber-pulse"></div>
                </h2>
                <div className="space-y-3">
                  <div>
                    <label className="block text-cyber-light text-sm mb-1">Model Name</label>
                    <input
                      type="text"
                      value={model1Name}
                  ...
                  <div>
                    <label className="block text-cyber-light text-sm mb-1">Model Type</label>
                    <Tabs value={model1Type} onValueChange={(v) => setModel1Type(v as any)}>
                      <TabsList className="w-full bg-cyber-secondary/50">
                        <TabsTrigger value="demo" className="flex-1 text-cyber-light text-sm">Demo</TabsTrigger>
                        <TabsTrigger value="function" className="flex-1 text-cyber-light text-sm">Function</TabsTrigger>
                        <TabsTrigger value="url" className="flex-1 text-cyber-light text-sm">URL</TabsTrigger>
                        <TabsTrigger value="file" className="flex-1 text-cyber-light text-sm">File</TabsTrigger>
                      </TabsList>
                  ...
                  {model1ValidationError && (
                    <Alert variant="destructive" className="bg-cyber-warning/20 border-cyber-warning text-cyber-light text-sm">
                      <AlertDescription>{model1ValidationError}</AlertDescription>
                    </Alert>
                  )}
                  {/* Model 1 Stats */}
                  <div className="mt-4">
                    <h3 className="text-base font-bold text-cyber-light mb-2">Model Statistics</h3>
          ...
          <div className="w-full lg:w-2/4">
            <div className="bg-cyber-primary/80 backdrop-blur-sm rounded-lg p-4 border border-cyber-accent/30 shadow-cyber">
              <div className="text-center mb-4">
                <h2 className="text-xl font-bold text-cyber-light mb-2">Game Status</h2>
                <div className="bg-cyber-secondary/30 rounded-lg p-3 border border-cyber-accent/20">
                  {showCoinFlip ? (
                    <p className="text-cyber-accent text-base animate-cyber-pulse">
                      Flipping coin to determine who goes first...
                    </p>
                  ) : gameOver ? (
                    <div className="space-y-1">
                      <p className="text-cyber-accent text-base font-medium">
                        {winResult.winner ? (
                          <span>
              ...
              <div className="relative">
                {/* Coin Flip */}
                {showCoinFlip && (
                  <div className="absolute inset-0 flex items-center justify-center bg-cyber-primary/90 backdrop-blur-sm z-10">
                    <div className={`coin ${flippedResult ? '' : 'coin-flipping'}`}>
                ...
                {gameOver && winResult.winner && (
                  <div className="winning-animation">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <h2 className={`victory-text ${winResult.winner === 1 ? 'text-cyber-accent' : 'text-cyber-accent-2'}`}>
                ...
                <div className="grid grid-cols-3 gap-2 max-w-sm mx-auto">
                  {board.map((cell, index) => (
                    <button
                      ...
                      className={`aspect-square bg-cyber-secondary/50 border border-cyber-accent/30 rounded-lg text-2xl font-bold flex items-center justify-center transition-all duration-300 ${getCellColor(cell)} ${
                        !isPlayerTurn || cell !== null
                          ? 'cursor-not-allowed opacity-50'
                          : 'hover:bg-cyber-secondary hover:border-cyber-accent hover:shadow-cyber-glow'
                      } ${
                ...
                {gameOver ? (
                  <div className="flex gap-4 justify-center">
                    <button
                      onClick={startGame}
                      disabled={isLoading}
                      className="flex-1 px-6 py-3 rounded-lg text-cyber-light font-bold text-base transform hover:scale-105 transition-all duration-300 bg-cyber-accent hover:bg-cyber-accent/80 shadow-cyber hover:shadow-cyber-glow"
                    >
                      Rematch
                    </button>
                    <button
                      ...
                      className="flex-1 px-6 py-3 rounded-lg text-cyber-light font-bold text-base transform hover:scale-105 transition-all duration-300 bg-cyber-secondary hover:bg-cyber-secondary/80 shadow-cyber hover:shadow-cyber-glow"
                    >
                      Select Models
                    </button>
                  </div>
                  ...
                  <div className="flex gap-4 justify-center">
                    <div className="flex-1 flex gap-4">
                      {!gameInProgress && !gameOver && (
                        <div className="relative">
                          <label className="absolute -top-6 left-0 text-xs text-cyber-accent font-medium">
                            Match Count
                          </label>
                          <select
                            value={matchCount}
                            onChange={(e) => setMatchCount(Number(e.target.value) as 1 | 5 | 10 | 20)}
                            className="w-32 bg-cyber-secondary/50 border border-cyber-accent/30 rounded-lg px-4 py-3 text-cyber-light text-base focus:outline-none focus:border-cyber-accent focus:shadow-cyber-glow transition-all duration-300 appearance-none cursor-pointer hover:border-cyber-accent hover:shadow-cyber-glow"
                          >
                      ...
                      <button
                        ...
                        disabled={isLoading}
                        className={`flex-1 px-8 py-3 rounded-lg text-cyber-light font-bold text-base transform hover:scale-105 transition-all duration-300 ${
                          isLoading
                            ? 'bg-cyber-secondary/50 cursor-not-allowed'
                            : 'bg-cyber-accent hover:bg-cyber-accent/80 shadow-cyber hover:shadow-cyber-glow'
                        }`}
                      >
                        {isLoading ? (
                          <span className="flex items-center justify-center">
                            <svg className="animate-cyber-spin h-4 w-4 mr-2" viewBox="0 0 24 24">
                    ...
                    {!gameInProgress && !gameOver && (
                      <button
                        ...
                        className="flex-1 px-8 py-3 rounded-lg text-cyber-light font-bold text-base transform hover:scale-105 transition-all duration-300 bg-cyber-warning/20 hover:bg-cyber-warning/30 border border-cyber-warning/50 hover:border-cyber-warning shadow-cyber hover:shadow-cyber-glow"
                      >
                        Clear Data
                      </button>
                    )}
          ...
          {/* Right Side - Model O Config or Stats */}
          <div className="w-full lg:w-1/4">
            {!gameInProgress && !gameOver ? (
              <div className="bg-cyber-primary/80 backdrop-blur-sm rounded-lg p-4 border border-cyber-accent-2/30 shadow-cyber-glow-pink hover:shadow-cyber-glow-pink hover:animate-cyber-glow-pink transition-all duration-300">
                <h2 className="text-xl font-bold text-cyber-light mb-3 flex items-center">
                  <span className="text-cyber-accent-2 mr-2">Model O</span>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-cyber-secondary/50 border border-cyber-accent-2/30 rounded px-3 py-1.5 text-cyber-light text-sm focus:outline-none focus:border-cyber-accent-2 focus:shadow-cyber-glow-pink transition-all duration-300"
                      placeholder="Enter model name"
                    />
                  </div>
                  <div>
                    <label className="block text-cyber-light text-sm mb-1">Model Type</label>
                    <Tabs value={model2Type} onValueChange={(v) => setModel2Type(v as any)}>
                      <TabsList className="w-full bg-cyber-secondary/50">
                        <TabsTrigger value="demo" className="flex-1 text-cyber-light text-sm">Demo</TabsTrigger>
                        <TabsTrigger value="function" className="flex-1 text-cyber-light text-sm">Function</TabsTrigger>
                        <TabsTrigger value="url" className="flex-1 text-cyber-light text-sm">URL</TabsTrigger>
                      ...
                      <TabsContent value="function" className="pt-2">
                        <Textarea 
                          ...
                          className="min-h-[100px] font-mono text-xs bg-cyber-secondary/50 text-cyber-light border-cyber-accent-2/30 focus:border-cyber-accent-2 focus:shadow-cyber-glow-pink transition-all duration-300"
                        />
                      </TabsContent>
                      <TabsContent value="url" className="pt-2">
                        <Input
                          ...
                          value={model2Input}
                          onChange={(e) => setModel2Input(e.target.value)}
                          className="bg-cyber-secondary/50 text-cyber-light text-sm border-cyber-accent-2/30 focus:border-cyber-accent-2 focus:shadow-cyber-glow-pink transition-all duration-300"
                        />
                      </TabsContent>
                      <TabsContent value="file" className="pt-2">
                        <ModelFileUpload 
                  ...
                  {model2ValidationError && (
                    <Alert variant="destructive" className="bg-cyber-warning/20 border-cyber-warning text-cyber-light text-sm">
                      <AlertDescription>{model2ValidationError}</AlertDescription>
                    </Alert>
                  )}
                  {/* Model 2 Stats */}
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Wins</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.wins}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Losses</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.losses}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Draws</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.draws}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Win Rate</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">
                      ...
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Invalid Moves</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.invalidMoves}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Timeouts</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.timeouts}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Crashes</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.crashes}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Total Moves</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.totalMoves}</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Total Time</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.totalTime.toFixed(0)}ms</p>
                      </div>
                      <div className="bg-cyber-secondary/50 rounded-lg p-2">
                        <p className="text-cyber-light text-xs">Avg. Time</p>
                        <p className="text-cyber-accent-2 text-lg font-bold">{model2Stats.averageTime.toFixed(0)}ms</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-cyber-primary/80 backdrop-blur-sm rounded-lg p-4 border border-cyber-accent-2/30 shadow-cyber-glow-pink hover:shadow-cyber-glow-pink hover:animate-cyber-glow-pink transition-all duration-300">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-bold text-cyber-accent-2">{model2Name || "Model O"}</h3>
